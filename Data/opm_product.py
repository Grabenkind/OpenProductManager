...
def product_add():
    ...
    try:
        ...
        command = """INSERT INTO products (ItemNo,
                    ...
                    VALUES (%d, "%s", "%s", %d, %d);""" % (itemnumber, name, brand, price, quantity)
        c.execute(command)
        db.commit()
    except Error as e:
        print(e)
...
def product_edit_menu():
    ...
    print("| b - EDIT brand of a product          |")
...
def product_edit_brand():
    try:
        ...
        newbrand = str(input("Brand: "))

        db = sqlite3.connect('products.db')
        c = db.cursor()
        command = """UPDATE products SET Brand = "%s"
        WHERE ItemNo = %d;""" % (newbrand, itemnumber)
        c.execute(command)
        db.commit()
    except Error as e:
        print(e)
